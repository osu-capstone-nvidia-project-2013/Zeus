...
private:
	void DrawScene(const Camera& camera, bool drawSphere, bool drawSkull, bool mainDraw);
...
private:
	...
	static const int CubeMapSizeSphere = 1024;
	static const int CubeMapSizeSkull = 1024;
	static const int CubeMapSizeMirror = 2048;
...
: D3DApp(hInstance), mSky(0), mWalkCamMode(true), mShapesVB(0), mShapesIB(0), mSkullVB(0), mSkullIB(0), 
  ...
  mDynamicCubeMapSRVSkull(0), mDynamicCubeMapDSVMirror(0), mDynamicCubeMapSRVMirror(0), mSkullIndexCount(0),  
  mRenderOptions(RenderOptionsBasic), mSmap(0), mLightRotationAngle(0.0f)
{
	...
	mCam.SetPosition(0.0f, 4.5f, -10.0f);
	...
	XMMATRIX boxOffset = XMMatrixTranslation(0.0f, 1.5f, 0.0f);
	XMStoreFloat4x4(&mBoxWorld, XMMatrixMultiply(boxScale, boxOffset));

	// Alligned cylinders and spheres
	for(int i = 0; i < 2; ++i)
	{
		XMStoreFloat4x4(&mCylWorld[i*2+0], XMMatrixTranslation(-5.0f, 2.5f, -10.0f + i*5.0f));
		XMStoreFloat4x4(&mCylWorld[i*2+1], XMMatrixTranslation(+5.0f, 2.5f, -10.0f + i*5.0f));

		XMStoreFloat4x4(&mSphereWorld[i*2+0], XMMatrixTranslation(-5.0f, 5.0f, -10.0f + i*5.0f));
		XMStoreFloat4x4(&mSphereWorld[i*2+1], XMMatrixTranslation(+5.0f, 5.0f, -10.0f + i*5.0f));
	}

	// Fallen Sphere
	XMStoreFloat4x4(&mCylWorld[2*2+0], XMMatrixTranslation(-5.0f, 2.5f, 10.0f));
	XMStoreFloat4x4(&mSphereWorld[2*2+0], XMMatrixTranslation(-3.5f, 2.0f, 9.5f));

    // Trees
	XMMATRIX treeScale = XMMatrixScaling(5.0f, 5.0f, 5.0f);
	XMMATRIX treeOffset = XMMatrixTranslation(10.0f, 0.0f, 50.0f);
	XMStoreFloat4x4(&mObjWorld[0], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(0.0f, 0.0f, 30.0f);
	XMStoreFloat4x4(&mObjWorld[1], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(-10.0f, 0.0f, 75.0f);
	XMStoreFloat4x4(&mObjWorld[2], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(50.0f, 0.0f, 10.0f);
	XMStoreFloat4x4(&mObjWorld[3], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(70.0f, 0.0f, 50.0f);
	XMStoreFloat4x4(&mObjWorld[4], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(80.0f, 0.0f, -30.0f);
	XMStoreFloat4x4(&mObjWorld[5], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(30.0f, 0.0f, -10.0f);
	XMStoreFloat4x4(&mObjWorld[6], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(55.0f, 0.0f, 60.0f);
	XMStoreFloat4x4(&mObjWorld[7], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(85.0f, 0.0f, 40.0f);
	XMStoreFloat4x4(&mObjWorld[8], XMMatrixMultiply(treeScale, treeOffset));
	treeOffset = XMMatrixTranslation(20.0f, 0.0f, 20.0f);
	XMStoreFloat4x4(&mObjWorld[9], XMMatrixMultiply(treeScale, treeOffset));
	...
	mObjMat.Ambient  = XMFLOAT4(0.6f, 0.4f, 0.2f, 1.0f);
	mObjMat.Diffuse  = XMFLOAT4(0.6f, 0.45f, 0.2f, 1.0f);
	mObjMat.Specular = XMFLOAT4(0.4f, 0.4f, 0.4f, 4.0f);
...
bool ZeusApp::Init()
{
	...
	HR(mFont.Initialize(md3dDevice, L"Perpetua", 48.0f, FontSheet::FontStyleRegular, true));
...
void ZeusApp::UpdateScene(float dt)
{
	...
	// Cow 
	XMMATRIX skullScale = XMMatrixScaling(1.5f, 1.5f, 1.5f);
	XMMATRIX skullOffset = XMMatrixTranslation(3.5f, 3.5f, 0.0f);
...
void ZeusApp::DrawScene()
{
	...
	{
		BuildCubeFaceCamera(5.0f, 2.5f, 5.0f); // Sphere position
        ...
        DrawScene(mCubeMapCamera[i], false, true, false);
	}
	...
	for(int i = 0; i < 6; ++i) // for mirror, just do (int i = 0; i < 1; ++i) for 1 camera mapped to mirror surface
	{
		...
		// Draw the scene with the exception of the center sphere to this cube map face.
		DrawScene(mCubeMapCamera[i], true, true, false);
	}
	...
	DrawScene(mCam, true, true, true); // Switch to false if turning off dynamic cube mapping

	HR(mSwapChain->Present(0, 0));
}
	

void ZeusApp::DrawScene(const Camera& camera, bool drawSphere, bool drawSkull, bool mainDraw)
{
	...
	for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		for(int i = 0; i < 10; i++)
		{
			world = XMLoadFloat4x4(&mObjWorld[i]);
			...
			md3dImmediateContext->DrawIndexed(mObjIndexCount, 0, 0);
		}
	}
	...
	float blenderFactor[4] = {1.0f};
	md3dImmediateContext->OMSetBlendState(RenderStates::TransparentBS, blenderFactor, 0xffffffff);

	std::wstring text = L"Demo";
	std::wstring hair = L"+";
    if(mainDraw)
    {
	...
	// Center the text in the top screen.
	POINT textPos;
	textPos.x = (mClientWidth - textWidth) - 1.0;
	textPos.y = 1.0;
	...
	mSpriteBatch.DrawString(md3dImmediateContext, mFont, hair, hairPos, XMCOLOR(0xf0000000));
    }

	// restore default
	md3dImmediateContext->OMSetBlendState(0, blenderFactor, 0xffffffff);
}
...
void ZeusApp::BuildShadowTransform()
{
...
void ZeusApp::LoadObjectBuffers()
{
	...
	D3D11_BUFFER_DESC ibd;
    ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(UINT) * mObjIndexCount;
    ibd.BindFlags = D3D11_BIND_INDEX_BUFFER;
    ibd.CPUAccessFlags = 0;
    ibd.MiscFlags = 0;
    D3D11_SUBRESOURCE_DATA iinitData;
