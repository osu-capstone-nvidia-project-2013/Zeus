...
HRESULT SpriteBatch::Initialize(ID3D11Device* device)
{
	// Prevent double Init.
	assert(!mInitialized);
	...
	hr = device->CreateInputLayout(layoutDesc, 3, passDesc.pIAInputSignature,
...
void SpriteBatch::BeginBatch(ID3D11ShaderResourceView* texSRV)
{
	assert(mInitialized);
...
void SpriteBatch::EndBatch(ID3D11DeviceContext* dc)
{
	assert(mInitialized);
...
void SpriteBatch::Draw(const POINT& position, XMCOLOR color)
{
	Sprite sprite;
...
void SpriteBatch::Draw(const POINT& position, const CD3D11_RECT& sourceRect, XMCOLOR color)
{
...
void SpriteBatch::Draw(const CD3D11_RECT& destinationRect, XMCOLOR color)
{
	Sprite sprite;
	sprite.SrcRect  = CD3D11_RECT(0, 0, mTexWidth, mTexHeight);
...
void SpriteBatch::Draw(const CD3D11_RECT& destinationRect, const CD3D11_RECT& sourceRect, XMCOLOR color)
{
	Sprite sprite;
	sprite.SrcRect  = sourceRect;
	sprite.DestRect = destinationRect;
...
void SpriteBatch::DrawString(ID3D11DeviceContext* dc, FontSheet& fs, const std::wstring& text, 
...
void SpriteBatch::DrawBatch(ID3D11DeviceContext* dc, UINT startSpriteIndex, UINT spriteCount)
{
	// 
